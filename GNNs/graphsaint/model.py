...
class GraphSAINT:
    ...
    def _loss(self):
        ...
        self.accuracyloss = self.loss
        self.fairnessloss = tf.convert_to_tensor(0.0,dtype=tf.float32)
        if FLAGS.fair:
            if FLAGS.loss_type == 'dp': 
                # debias objective

                max_index = tf.dtypes.cast(tf.math.argmax(self.node_preds,axis=1), tf.int32) + 1 # index start from one
                max_value = tf.math.reduce_max(tf.nn.softmax(self.node_preds,axis=1), axis=1)
                
                # male
                max_male_index = max_index * self.male_mask

                
                N_i_list = []
                N_list = []
                for i in range(1,self.num_classes+1): # index start from one
                    N_male_i = tf.reduce_sum(tf.cast(tf.math.equal(max_male_index, i), tf.float32)*max_value) + 1
                    N_i_list.append(N_male_i)
                    N_list.append(tf.reduce_sum(self.male_mask) + self.num_classes) 
                male_dist = tf.math.divide_no_nan(N_i_list,N_list)
                # female
                max_female_index = max_index * self.female_mask
                
                N_i_list = []
                N_list = []
                for i in range(1,self.num_classes+1):
                    N_female_i = tf.reduce_sum(tf.cast(tf.math.equal(max_female_index, i), tf.float32)*max_value) + 1
                    N_i_list.append(N_female_i)
                    N_list.append(tf.reduce_sum(self.female_mask) + self.num_classes)
                female_dist = tf.math.divide_no_nan(N_i_list,N_list)

                self.fairnessloss = tf.dtypes.cast(tf.nn.l2_loss(male_dist - female_dist),tf.float32)

                self.male_dist_print = male_dist # detail
                self.female_dist_print = female_dist # detail


            elif FLAGS.loss_type == 'eo':
                # debias objective  # multi-classes
                labels = self.placeholders['labels'] # n_batch x num_class
                gt_index = tf.dtypes.cast(tf.math.argmax(labels,axis=1), tf.int32) + 1
                
                max_index = tf.dtypes.cast(tf.math.argmax(self.node_preds,axis=1), tf.int32) + 1 # index start from one
                max_value = tf.math.reduce_max(tf.nn.softmax(self.node_preds,axis=1), axis=1)
                
                correct_mask = tf.dtypes.cast(tf.math.equal(max_index, gt_index), tf.int32)
                correct_index = max_index * correct_mask
                
                # self.gt_index = gt_index # detail
                # self.max_index = max_index # detail
                # self.max_value = max_value # detail
                # self.correct_mask = correct_mask # detail
                # self.correct_index = correct_index # detail
                ...
                N_list = []
                for i in range(1,self.num_classes+1): # index start from one
                    N_male_i = tf.reduce_sum(tf.cast(tf.math.equal(correct_index*self.male_mask, i), tf.float32)*max_value)
                    N = tf.reduce_sum(tf.cast(tf.math.equal(gt_index*self.male_mask, i), tf.float32))
                    N_i_list.append(N_male_i)
                    N_list.append(N)
                
                male_dist = tf.math.divide_no_nan(N_i_list,N_list)

                # N_i_list = tf.convert_to_tensor(N_i_list, dtype=tf.float32) # detail
                # N_list = tf.convert_to_tensor(N_list, dtype=tf.float32) # detail
                
                # self.male_dist_print = male_dist # detail
                # self.N_i_list = N_i_list # detail
                # self.N_list = N_list # detail
                ...
                N_list = []
                for i in range(1,self.num_classes+1): # index start from one
                    N_female_i = tf.reduce_sum(tf.cast(tf.math.equal(correct_index*self.female_mask, i), tf.float32)*max_value)
                    N = tf.reduce_sum(tf.cast(tf.math.equal(gt_index*self.female_mask, i), tf.float32))
                    N_i_list.append(N_female_i)
                    N_list.append(N)
                female_dist = tf.math.divide_no_nan(N_i_list,N_list)
                self.fairnessloss = tf.dtypes.cast(tf.nn.l2_loss(male_dist - female_dist),tf.float32)
            ...
            self.loss += self.alpha * self.fairnessloss
        
        tf.summary.scalar('loss', self.loss)
        tf.summary.scalar('fairnessloss', self.fairnessloss)
