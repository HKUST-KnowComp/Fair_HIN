...
class Minibatch:
    ...
    def __init__(self, adj_full, adj_full_norm, adj_train, role, class_arr, placeholders, train_params, all_male_users, all_female_users, **kwargs):
        """
        ...
        self.num_proc = 10
        self.node_page = np.array(role['page']) # train user + page
        self.node_train = np.array(role['tr'])
        self.node_val = np.array(role['va'])
        self.node_test = np.array(role['te'])
        self.node_all = np.array(role['tr']+role['va']+role['te'])

        self.node_user = np.array(list((set(self.node_train)|set(self.node_val)|set(self.node_test))-set(self.node_page)),dtype=int)
        print('zzq','num page', self.node_page.size, 'num train', self.node_train.size, 'num val', self.node_val.size, 'node test',self.node_test.size)
        ...
        self.adj_train = copy.deepcopy(adj_full) 
        ...
        self.male_users = all_male_users
        self.female_users = all_female_users

        self.male_mask = np.zeros(len(self.node_train) + len(self.node_val) +  len(self.node_test))
        self.female_mask = np.zeros(len(self.node_train) + len(self.node_val) +  len(self.node_test))
        
        self.male_mask[self.male_users] = 1
        self.female_mask[self.female_users] = 1

        print('male_users',type(self.male_users),len(self.male_users),self.male_users)
        print('female_users',type(self.female_users),len(self.female_users),self.female_users)
        print('male_mask',type(self.male_mask),len(self.male_mask),self.male_mask)
        print('female_mask',type(self.female_mask),len(self.female_mask),self.female_mask)
        print('node_page',len(self.node_page),self.node_page[-10:])
        print('male_mask page',np.any(self.male_mask[self.node_page]))
        print('female_mask page',np.any(self.female_mask[self.node_page]))
    ...
    def set_sampler(self,train_phases):
        ...
        elif self.method_sample == 'fbrw':
            self.size_subg_budget = train_phases['num_root']*train_phases['depth']
            self.graph_sampler = fbrw_sampling(self.adj_train,\
                self.node_train,self.size_subg_budget,int(train_phases['num_root']),int(train_phases['depth']),self.node_user)
        elif self.method_sample == 'brw':
            self.size_subg_budget = train_phases['num_root']*train_phases['depth']
            self.graph_sampler = brw_sampling(self.adj_train,\
                self.node_train,self.size_subg_budget,int(train_phases['num_root']),int(train_phases['depth']),self.node_user,self.adj_bias_data)
        ...
        self.norm_loss_train[self.node_all] = num_subg/self.norm_loss_train[self.node_all]/self.node_all.size
        self.norm_loss_train[self.node_val] = 0
        self.norm_loss_train[self.node_test] = 0
        self.norm_loss_train[self.node_page] = 0
        print()
    ...
    def feed_dict(self,mode='train'):
        ...
        male_mask = self.male_mask[self.node_subgraph] 
        female_mask = self.female_mask[self.node_subgraph]
        
        # print('male_mask',len(male_mask),male_mask[:10])
        # print('female_mask',len(female_mask),female_mask[:10])
        feed_dict.update({self.placeholders['male_mask']: male_mask})
        feed_dict.update({self.placeholders['female_mask']: female_mask})
        ...
        feed_dict[self.placeholders['alpha']] = self.alpha
        feed_dict[self.placeholders['gamma']] = self.gamma
        feed_dict[self.placeholders['lr']] = self.lr
    ...
    def set_lr_time_based_decay(self, decay, epoch):
        self.lr = self.lr * ( 1.0 / ( 1.0 + decay*epoch ) )
        return self.lr

    def set_lr_step_decay(self, epoch, initial_lr=0.0001, epochs_drop=20, drop=0.5):
        self.lr = initial_lr * math.pow(drop,  math.floor((1+epoch)/epochs_drop))
        return self.lr

    def set_lr_exp_decay(self, epoch,  initial_lr=0.001,  k=0.05):
        self.lr = initial_lr * math.exp(-k*epoch)
        return self.lr
