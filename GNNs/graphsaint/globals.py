...
flags.DEFINE_float('alpha', 1.0, 'fairness trade off hyperparameter')
flags.DEFINE_float('disc_lr', 0.01, 'fairness trade off hyperparameter')
flags.DEFINE_string('loss_type', 'dp', 'fairness trade off hyperparameter')
flags.DEFINE_string('fair_level', 'high', 'fairness trade off hyperparameter')
flags.DEFINE_integer('inner_no', 0, 'fairness trade off hyperparameter')
flags.DEFINE_integer('outer_no', 0, 'fairness trade off hyperparameter')
# flags.DEFINE_integer('t_e', 0, 'fairness trade off hyperparameter')
flags.DEFINE_boolean('fair', True, "Whether to use fair-aware loss.")
# flags.DEFINE_string('emb_dir', '', "/data/zzengae/code_metapath2veec/gcn_emb/fb")
# Settings
flags.DEFINE_float('gamma', 1.0, 'adversarial loss trade off hyperparameter')
flags.DEFINE_integer('num_cpu_core', 20, 'Number of CPU cores for parallel sampling')
flags.DEFINE_boolean('log_device_placement', False, "Whether to log device placement.")
flags.DEFINE_string('data_prefix', 'MovieLens', 'prefix identifying training data. must be specified.')
flags.DEFINE_string('dataset', 'ml', 'MovieLens or Facebook')
flags.DEFINE_string('thres_file','thres_fair_conditions.json','threshold values of dp/eo under high/med/low fair conditions')
flags.DEFINE_string('dir_log', 'log', 'base directory for logging and saving embeddings') # avoid naming the flag to `log_dir`, as it may cause conflict when using higher version of TF
flags.DEFINE_string('dir_output', 'output_result', 'the directory for saving output') 
...
flags.DEFINE_string('train_config', 'train_config/ml_rw.yml', "path to the configuration of training (*.yml)")
