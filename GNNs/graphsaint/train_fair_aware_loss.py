...
def evaluate_full_batch(sess,model,minibatch_iter,many_runs_timeline,mode,final=False):
	...
	if FLAGS.timeline:
		preds,loss,acc_loss,fair_loss = sess.run([model.preds, model.loss, model.accuracyloss, model.fairnessloss], feed_dict=feed_dict, options=options, run_metadata=run_metadata)
		fetched_timeline = timeline.Timeline(run_metadata.step_stats)
		chrome_trace = fetched_timeline.generate_chrome_trace_format()
		many_runs_timeline.append(chrome_trace)
	else:
		preds,loss,acc_loss,fair_loss = sess.run([model.preds, model.loss, model.accuracyloss, model.fairnessloss], feed_dict=feed_dict)
	...
	return loss, mrr, f1_weight, dp, eo, acc_loss, fair_loss
...
def train(train_phases,arch_gcn,model,minibatch,\
	...
	for ip,phase in enumerate(train_phases):
		...
		if ip == 0:
			minibatch.set_alpha(0.0)
		elif ip == 1:
			minibatch.set_alpha(FLAGS.alpha)
			minibatch.set_lr(0.1)
		for e in range(epoch_ph_start,int(phase['end'])):
			...
			while not minibatch.end():
				...
				if FLAGS.timeline:      # profile the code with Tensorflow Timeline
					_,loss_train,acc_loss_train,pred_train,fair_loss_train,dbg = sess.run(
							[model.opt_op, model.loss, model.accuracyloss, model.preds, model.fairnessloss, model.alpha, model.lr], feed_dict=feed_dict,
							options=options, run_metadata=run_metadata)
					fetched_timeline = timeline.Timeline(run_metadata.step_stats)
					chrome_trace = fetched_timeline.generate_chrome_trace_format()
					many_runs_timeline.append(chrome_trace)
				else:
					# _,loss_train,acc_loss_train,pred_train,fair_loss_train = sess.run(
					# 	[model.opt_op, model.loss, model.accuracyloss, model.preds, model.fairnessloss, model.alpha, model.lr], \
					# 						feed_dict=feed_dict,options=tf.RunOptions(report_tensor_allocations_upon_oom=True))

					_,loss_train,acc_loss_train,pred_train,fair_loss_train,check_alpha,check_lr = sess.run(
						[model.opt_op, model.loss, model.accuracyloss, model.preds, model.fairnessloss, model.alpha, model.lr], \
											feed_dict=feed_dict,options=tf.RunOptions(report_tensor_allocations_upon_oom=True))
				...
				if not minibatch.batch_num % FLAGS.eval_train_every:
					...
					l_fair_loss_tr.append(check_alpha*fair_loss_train)
					l_acc_loss_tr.append(acc_loss_train)
					l_loss_tr.append(loss_train)
					l_mrr_tr.append(mrr)
					l_f1_weight_tr.append(f1_weight)
					l_size_subg.append(minibatch.size_subgraph)

			printf('Epoch {:4d}  Alpha {:4.1f}'.format(e,check_alpha),style='underline')
			...
			loss_val, mrr_val, f1_weight_val, dp_val, eo_val, acc_loss_val, fair_loss_val = \
				evaluate_full_batch(sess_eval,model,minibatch,many_runs_timeline,mode='val')
			loss_test, mrr_test, f1_weight_test, dp_test, eo_test, acc_loss_test, fair_loss_test = \
				evaluate_full_batch(sess_eval,model,minibatch,many_runs_timeline,mode='test')
			printf('TRAIN     : loss = {:.5f}\tmrr = {:.5f}\tw_mic = {:.5f}\taccuracy_loss = {:.5f}\tfairness_loss = {:.5f}'.format(
				f_mean(l_loss_tr),f_mean(l_mrr_tr),f_mean(l_f1_weight_tr),f_mean(l_acc_loss_tr),f_mean(l_fair_loss_tr)))
			printf('VALIDATION: loss = {:.5f}\tmrr = {:.5f}\tw_mic = {:.5f}\tdp = {:.5f}\teo = {:.5f}'.format(
				loss_val,mrr_val,f1_weight_val,dp_val,eo_val),style='yellow')
			
			if FLAGS.loss_type == 'dp':
				selected_fair_val = dp_val
				selected_fair_test = dp_test
			elif FLAGS.loss_type == 'eo':
				selected_fair_val = eo_val
				selected_fair_test = eo_test
			for fair_level in ['low','high','med']:
				if (selected_fair_val < threshold[FLAGS.loss_type]['dev_'+fair_level] \
				and selected_fair_test < threshold[FLAGS.loss_type]['test_'+fair_level] \
				and mrr_val > mrr_best[FLAGS.loss_type+'_'+fair_level]) or e == 0:
					mrr_best[FLAGS.loss_type+'_'+fair_level] = mrr_val
					if not os.path.exists(FLAGS.dir_log+'/models'):
						os.makedirs(FLAGS.dir_log+'/models')
					print('Saving models {} {}...'.format(FLAGS.loss_type,fair_level))
					savepath = saver.save(sess, '{}/models/saved_model_{}_{}_{}.chkpt'.format(FLAGS.dir_log,FLAGS.loss_type,fair_level,timestamp),write_meta_graph=False,write_state=False)
	...
	for fair_level in ['low','med','high']:
		saver.restore(sess_eval, '{}/models/saved_model_{}_{}_{}.chkpt'.format(FLAGS.dir_log,FLAGS.loss_type,fair_level,timestamp))
		loss_val, mrr_val, f1_weight_val, dp_val, eo_val, acc_loss_val, fair_loss_val = \
			evaluate_full_batch(sess_eval,model,minibatch,many_runs_timeline,mode='val')
		loss_test, mrr_test, f1_weight_test, dp_test, eo_test, acc_loss_test, fair_loss_test = \
			evaluate_full_batch(sess_eval,model,minibatch,many_runs_timeline,mode='test')
		...
		with open(join(FLAGS.dir_output,'fair_loss_{}_{}_{}.txt'.format(FLAGS.loss_type,fair_level,FLAGS.dataset)),'a') as fo:
			fo.write('alpha {:.2f} outer_no {:d} inner_no {:2d}\n'.format(
				FLAGS.alpha, FLAGS.outer_no, FLAGS.inner_no))
			if FLAGS.loss_type == 'dp':
	...
	for fair_level in ['low','med','high']:
		command = 'rm {}/models/saved_model_{}_{}_{}.chkpt*'.format(FLAGS.dir_log,FLAGS.loss_type, fair_level,timestamp)
		print(command)
		os.system(command)
