...
cdef extern from "<numeric>" namespace "std" nogil:
    OutputIt partial_sum[InputIt,OutputIt]( InputIt first, InputIt last, OutputIt d_first ) except +
...
cdef class BRW(Sampler):
    cdef int size_root, size_depth
    cdef vector[int] node_user_vec
    cdef vector[float] adj_bias_data_vec
    def __cinit__(self, np.ndarray[int,ndim=1,mode='c'] adj_indptr,
                        np.ndarray[int,ndim=1,mode='c'] adj_indices,
                        np.ndarray[int,ndim=1,mode='c'] node_train,
                        int num_proc, int num_sample_per_proc,
                        int size_root, int size_depth, 
                        np.ndarray[int,ndim=1,mode='c'] node_user,
                        np.ndarray[float,ndim=1,mode='c'] adj_bias_data):
        self.size_root = size_root
        self.size_depth = size_depth

        cutils.npy2vec_float(adj_bias_data,self.adj_bias_data_vec)
        cutils.npy2vec_int(node_user,self.node_user_vec)
    ...
    cdef void sample(self, int p) nogil:
        ...
        cdef int idx_subg
        cdef int v
        cdef float sample_thres
        cdef float rand_range_a_node
        cdef int num_train_node = self.node_train_vec.size()
        cdef int num_user_node = self.node_user_vec.size()
        cdef vector[float] partial_e_dist_cumsum_vec
        cdef vector[int] partial_neighbor_vec
        cdef int start_indptr = 0
        cdef int end_indptr = 0
        cdef int num_neighbor = 0
        while r < self.num_sample_per_proc:
            idx_subg = p*self.num_sample_per_proc+r
            # sample root
            iroot = 0
            while iroot < self.size_root:
                v = self.node_user_vec[rand()%num_user_node]
                self.node_sampled[idx_subg].push_back(v)
                # sample random walk
                idepth = 0
                ...
                # biased prob
                while idepth < self.size_depth:
                    start_indptr = self.adj_indptr_vec[v]
                    end_indptr = self.adj_indptr_vec[v+1]
                    num_neighbor = end_indptr - start_indptr

                    partial_e_dist_cumsum_vec.clear()
                    partial_e_dist_cumsum_vec.resize(end_indptr - start_indptr)
                    
                    partial_sum(self.adj_bias_data_vec.begin()+start_indptr, 
                                self.adj_bias_data_vec.begin()+end_indptr, 
                                partial_e_dist_cumsum_vec.begin())

                    partial_neighbor_vec.clear()
                    partial_neighbor_vec.assign(self.adj_indices_vec.begin()+start_indptr, self.adj_indices_vec.begin()+end_indptr)

                    rand_range_a_node = partial_e_dist_cumsum_vec[num_neighbor-1]
                    sample_thres = (<float> rand()) / RAND_MAX * rand_range_a_node
                    v = partial_neighbor_vec[lower_bound(partial_e_dist_cumsum_vec.begin(),partial_e_dist_cumsum_vec.end(),sample_thres)-partial_e_dist_cumsum_vec.begin()]
                    self.node_sampled[idx_subg].push_back(v)
