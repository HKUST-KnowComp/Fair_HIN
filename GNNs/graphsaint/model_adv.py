...
class GraphSAINT:
    ...
    def __init__(self, num_classes, placeholders, features,
        ...
        self.lr = placeholders['lr'] # self.lr = train_params['lr'] 
        self.disc_lr = FLAGS.disc_lr
        self.node_subgraph = placeholders['node_subgraph']
        self.alpha = placeholders['alpha']
        ...
        self.d_optim = tf.train.AdamOptimizer(learning_rate=self.disc_lr)
                      
        self.e_optim = tf.train.AdamOptimizer(learning_rate=self.lr)
    

        self.encoder_loss = 0
        self.disc_loss = 0
        
        self.encoder_opt_op = None
        self.disc_opt_op = None

        self.norm_loss = placeholders['norm_loss']
        self.is_train = placeholders['is_train']

        self.male_mask = placeholders['male_mask']
        self.female_mask = placeholders['female_mask']
    ...
    def discriminator(self):
        x = self.outputs
        input_dim = self.outputs.shape[-1]
        with tf.variable_scope('discriminator'):
            x = tf.layers.dense(x,input_dim*2,activation=partial(tf.nn.leaky_relu, alpha=0.2))#,name='d_dense_0'
            x = tf.nn.dropout(x,rate=0.3)
            x = tf.layers.dense(x,input_dim*4,activation=partial(tf.nn.leaky_relu, alpha=0.2))#,name='d_dense_1'
            x = tf.nn.dropout(x,rate=0.3)
            x = tf.layers.dense(x,input_dim*4,activation=partial(tf.nn.leaky_relu, alpha=0.2))#,name='d_dense_2'
            x = tf.nn.dropout(x,rate=0.3)
            x = tf.layers.dense(x,input_dim*2,activation=partial(tf.nn.leaky_relu, alpha=0.2))#,name='d_dense_3'
            x = tf.nn.dropout(x,rate=0.3)
            x = tf.layers.dense(x,input_dim*2,activation=partial(tf.nn.leaky_relu, alpha=0.2))#,name='d_dense_4'
            x = tf.nn.dropout(x,rate=0.3)
            x = tf.layers.dense(x,input_dim,activation=partial(tf.nn.leaky_relu, alpha=0.2))#,name='d_dense_5'
            x = tf.nn.dropout(x,rate=0.3)
            x = tf.layers.dense(x,input_dim,activation=partial(tf.nn.leaky_relu, alpha=0.2))#,name='d_dense_6'
            x = tf.nn.dropout(x,rate=0.3)
            x = tf.layers.dense(x,input_dim//2,activation=partial(tf.nn.leaky_relu, alpha=0.2))#,name='d_dense_7'
            x = tf.nn.dropout(x,rate=0.3)
            x = tf.layers.dense(x,2,activation=None)#,name='d_dense_8'
        
        self.disc_logits = x

        f_loss = tf.nn.softmax_cross_entropy_with_logits
        self.disc_loss = f_loss(labels=tf.one_hot(self.female_mask,2),logits=self.disc_logits)
        if len(self.disc_loss.shape) == 1:
            self.disc_loss = tf.reshape(self.disc_loss,(-1,1))
        self.check_disc_loss = self.disc_loss
        user_mask = tf.cast(tf.math.logical_or(tf.cast(self.male_mask,dtype=tf.bool), tf.cast(self.female_mask,dtype=tf.bool)),dtype=tf.float32)
        user_mask_avg = user_mask / tf.reduce_sum(user_mask)
        _loss_terms_weight = tf.linalg.matmul(tf.transpose(self.disc_loss),\
                    tf.reshape(user_mask_avg,(-1,1)))
        self.disc_loss = tf.reduce_sum(_loss_terms_weight)

        for var in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='discriminator'):
            self.disc_loss += self.weight_decay * tf.nn.l2_loss(var)

    def eval_disc_accuracy(self):
        user_mask = tf.cast(tf.math.logical_or(tf.cast(self.male_mask,dtype=tf.bool), tf.cast(self.female_mask,dtype=tf.bool)),dtype=tf.float32)
        user_mask_avg = user_mask / tf.reduce_sum(user_mask)
        preds = tf.math.argmax(self.disc_logits,axis=-1,output_type=tf.dtypes.int32)
        return tf.reduce_sum(tf.cast(tf.math.equal(preds,self.female_mask),dtype=tf.float32)*user_mask_avg)
    ...
    def build(self, model_pretrain=None):
        ...
        with tf.control_dependencies(update_ops):
            grads_and_vars = self.e_optim.compute_gradients(self.encoder_loss-self.gamma*self.disc_loss) 
            clipped_grads_and_vars = []
            for grad, var in grads_and_vars:
                if 'discriminator' not in var.name:
                    clipped_grads_and_vars.append((tf.clip_by_value(grad, -5.0, 5.0) if grad is not None else None, var))
            self.encoder_var_name = tf.constant([var.name for grad, var in clipped_grads_and_vars]) 
            self.encoder_grad, _ = clipped_grads_and_vars[0]
            self.encoder_opt_op = self.e_optim.apply_gradients(clipped_grads_and_vars)
        

        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        with tf.control_dependencies(update_ops):
            grads_and_vars = self.d_optim.compute_gradients(self.gamma*self.disc_loss)
            # self.disc_var_name = tf.constant([var.name for grad, var in grads_and_vars]) 
            ...
            for grad, var in grads_and_vars:
                if 'discriminator' in var.name:
                    clipped_grads_and_vars.append((tf.clip_by_value(grad, -5.0, 5.0) if grad is not None else None, var))
            self.disc_var_name = tf.constant([var.name for grad, var in clipped_grads_and_vars]) 
            self.disc_grad, _ = clipped_grads_and_vars[0]
            self.disc_opt_op = self.d_optim.apply_gradients(clipped_grads_and_vars)

        self.encoder_preds = self.predict()
        self.disc_acc = self.eval_disc_accuracy()
    
    def _encoder_loss(self):
        # these are all the trainable var
        for aggregator in self.aggregators:
            for var in aggregator.vars.values():
                self.encoder_loss += self.weight_decay * tf.nn.l2_loss(var)
        ...
        self.loss_terms = f_loss(labels=self.placeholders['labels'],logits=self.node_preds)
        ...
        self.encoder_loss += tf.reduce_sum(_loss_terms_weight)
