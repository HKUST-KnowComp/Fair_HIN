...
//added 
std::vector<long long> c_table(0), u_table(0), p_table(0);
...
void NodeType() {
  ...
  for (i = 0; i < table_size; i++) {
    ...
    if (vocab[target].word[0] == 'c') {
      c_table.push_back(target);
    } else if (vocab[target].word[0] == 'u') {
      u_table.push_back(target);
    } else if (vocab[target].word[0] == 'p') {
      p_table.push_back(target);
    }
  }
  std::cout << "table_size     " << table_size << std::endl;
  std::cout << "c_table.size() " << c_table.size() << std::endl;
  std::cout << "u_table.size() " << u_table.size() << std::endl;
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) { //  
          if (d == 0) {
            ...
            if (pp) {
              if        (vocab[last_word].word[0] == 'c') {
                target = c_table[(next_random >> 16) % c_table.size()];
              } else if (vocab[last_word].word[0] == 'u') {
                target = u_table[(next_random >> 16) % u_table.size()];
