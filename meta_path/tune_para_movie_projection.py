...
--data_dir {} --b_c".format(
    ...
    for outer_no in range(args.num_folds_outer):
        for inner_no in range(args.num_folds_inner):
            for k in range(5):
                command = "CUDA_VISIBLE_DEVICES={} python eval_link_prediction_movie_projection_fair_constraint.py --emb {} \
        --emb_dir {} --data_dir {} --num_folds_inner {} --inner_no {} --outer_no {} --method m2v_bias_projection >> {}".format(
                    args.gpu,
                    emb_file,
                    args.emb_dir,
                    args.data_dir,
...
--data_dir {} --c".format(
    ...
    for outer_no in range(args.num_folds_outer):
        for inner_no in range(args.num_folds_inner):
            for k in range(5):
                command = "CUDA_VISIBLE_DEVICES={} python eval_link_prediction_movie_projection_fair_constraint.py --emb {} \
        --emb_dir {} --data_dir {} --num_folds_inner {} --inner_no {} --outer_no {} --method m2v_default_projection >> {}".format(
                    args.gpu,
                    emb_file,
                    args.emb_dir,
                    args.data_dir,
...
if __name__ == "__main__":
    ...
    if args.method == 'default':
        args.result_dir = 'result_movie_default_projection_cv'
        if not exists(args.result_dir):
            os.makedirs(args.result_dir)
        for num_walks in [50,100,150,200]: # 50,100,150,200
            for walk_len in [50,100,150,200]: # 50,100,150,200
                train_default(num_walks,walk_len)
    elif args.method == 'bias':
        args.result_dir = 'result_movie_bias_projection_cv'
        if not exists(args.result_dir):
            os.makedirs(args.result_dir)
